...
/**
 * vtiger Core Installer - A Composer to install vtiger in a webroot subdirectory
...
namespace Tests\javanile\Composer\phpunit;
...
use javanile\Composer\vtigerCoreInstaller;
use PHPUnit\Framework\TestCase;

class vtigerCoreInstallerTest extends TestCase {

	public function testSupports() {
		$installer = new vtigerCoreInstaller( new NullIO(), $this->createComposer() );

		$this->assertTrue( $installer->supports( 'vtiger-core' ) );
		$this->assertFalse( $installer->supports( 'not-vtiger-core' ) );
	}

	public function testDefaultInstallDir() {
		$installer = new vtigerCoreInstaller( new NullIO(), $this->createComposer() );
		$package   = new Package( 'javanile/test-package', '1.0.0.0', '1.0.0' );

		$this->assertEquals( 'vtiger', $installer->getInstallPath( $package ) );
	}
	...
	public function testSingleRootInstallDir() {
		...
		$rootPackage->setExtra( array(
			'vtiger-install-dir' => $installDir,
		) );
		$installer = new vtigerCoreInstaller( new NullIO(), $composer );
	...
	public function testCorePackageCanDefineInstallDirectory() {
		...
		$package->setExtra( array(
			'vtiger-install-dir' => 'not-vtiger',
		) );

		$this->assertEquals( 'not-vtiger', $installer->getInstallPath( $package ) );
	}
	...
	public function testSensitiveInstallDirectoriesNotAllowed( $directory ) {
		$this->jpbExpectException(
			'\InvalidArgumentException',
			'/Warning! .+? is an invalid vtiger install directory \(from test\/package\)!/',
			true
		);
		$composer  = $this->createComposer();
		$installer = new vtigerCoreInstaller( new NullIO(), $composer );
		$package   = new Package( 'test/package', '1.1.0.0', '1.1' );
		$package->setExtra( array( 'vtiger-install-dir' => $directory ) );
	...
	public static function resetInstallPaths() {
		$prop = new \ReflectionProperty( '\javanile\Composer\vtigerCoreInstaller', '_installedPaths' );
