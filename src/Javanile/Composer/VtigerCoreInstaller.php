...
namespace javanile\Composer;

use Composer\Config;
use Composer\Installer\LibraryInstaller;
use Composer\Package\PackageInterface;

class vtigerCoreInstaller extends LibraryInstaller {

	const TYPE = 'vtiger-core';

	const MESSAGE_CONFLICT = 'Two packages (%s and %s) cannot share the same directory!';
	const MESSAGE_SENSITIVE = 'Warning! %s is an invalid vtiger install directory (from %s)!';
	...
	public function getInstallPath( PackageInterface $package ) {
		...
		if ( $this->composer->getPackage() ) {
			...
			if ( ! empty( $topExtra['vtiger-install-dir'] ) ) {
				$installationDir = $topExtra['vtiger-install-dir'];
		...
		if ( ! $installationDir && ! empty( $extra['vtiger-install-dir'] ) ) {
			$installationDir = $extra['vtiger-install-dir'];
		}
		if ( ! $installationDir ) {
			$installationDir = 'vtiger';
		}
